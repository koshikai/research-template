...
def main():
        ...
        for file in files:
            ...
            if file_path.suffix in extensions or file_path.name == "uv.lock":
                try:
                    ...
                    has_snake = old_name in content
                    has_kebab = old_kebab in content

                    if has_snake or has_kebab:
                        print(f"Updating content: {file_path}")
                        new_content = content
                        if has_snake:
                            new_content = new_content.replace(old_name, new_name)
                        if has_kebab:
                            new_content = new_content.replace(old_kebab, new_kebab)
    ...
    print("Recommended: Run 'git add .' to stage all renamed files and directories.")
