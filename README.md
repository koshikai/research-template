...
> [!IMPORTANT]
> **使用開始前の必須手順**  
> 本リポジトリをテンプレートとして使用し開発を始める際は、まず最初に以下のコマンドでプロジェクト名を適切な名前（例: `my_project`）にリネームしてください。
> ```bash
> uv run scripts/rename_project.py my_project
> uv sync
> ```
> デフォルトの `ai_research_template` のままモジュールを使用すると、警告（`UserWarning`）が発生します。
